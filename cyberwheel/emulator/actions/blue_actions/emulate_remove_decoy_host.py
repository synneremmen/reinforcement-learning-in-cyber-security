"""
Module defines the class to remove a decoy in the emulator.

Note decoys are currenlty pre-deployed when starting an experiment.
This action emulates removing a decoy by turning off the network interface.
Once turned off, the decoy machine will be unreachable to other machines in the subnet.
"""
...
class EmulateRemoveDecoyHost(
    ...
    def build_emulator_cmd(self, decoy_hostname: str) -> str:
        """
        Construct shell command to execute remove decoy host in firewheel.
        This comand turn off the network interface which will allow the (pre-made)
        decoy to be visible to other hosts.
        ...
        action_cmd_arr = [
            ...
            f"'echo ubuntu | sudo -S ip link set {interface} down'",
        ]
